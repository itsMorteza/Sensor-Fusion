...
def reg_metricV2(value_idx, reg_channels):

    def compute_metricv2(y_true, y_pred):
        mask =  tf.reshape(tf.equal(y_true[...,-1], 1.0), shape=(-1,))
        pos_count = tf.reduce_sum(tf.cast(mask, tf.float32))
    ...
    if value_idx == 0:
        compute_metricv2.__name__ = 'x_offset'
    elif value_idx == 1:
        compute_metricv2.__name__ = 'y_offset'
    elif value_idx == 2:
        compute_metricv2.__name__ = 'log_w'
    elif value_idx == 3:
        compute_metricv2.__name__ = 'log_h'
    return compute_metricv2    
