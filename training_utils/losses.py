...
def smooth_l1_lossV2(sigma, reg_channels, data_format='channels_last'):
    ...
    def compute_smooth_l1_loss(y_true, y_pred):
        ...
        mask = tf.reshape(tf.equal(y_true, 1.0), shape=(-1, reg_channels)) 
        pos_count = tf.reduce_sum(tf.cast(mask, tf.float32))

        #y_true = y_true[...,:-1]

        y_true_masked = tf.boolean_mask(tf.reshape(y_true, shape=(-1, reg_channels)), mask=mask, axis=0)
        y_pred_masked = tf.boolean_mask(tf.reshape(y_pred,shape=(-1, reg_channels)), mask=mask, axis=0)                                
